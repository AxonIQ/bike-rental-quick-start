...
/**
 * Thin client over the Axon Server <em>Integration</em> HTTP API: send commands and queries, publish
 * and read events, and register this application's handler endpoints so Axon Server pushes messages
 * to them. All calls target a single {@code context} and are plain blocking HTTP, so callers run them
 * off the request/event-loop threads.
 */
@Component
