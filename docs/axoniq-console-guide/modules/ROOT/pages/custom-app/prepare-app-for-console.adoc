...
image::ac-connect-app-or-start-demo.png[alt="Screenshot of the AxonIQ Console dialog showing two options for instructions on how to connect an application. On the left side, there is a blue button to 'Connect my application' and on the right side, there is a blue button with the text 'Start demo'."]
...
You can explore a little bit the information provided in the different tabs, but as the application is idle, the information will not be very useful at this time.
